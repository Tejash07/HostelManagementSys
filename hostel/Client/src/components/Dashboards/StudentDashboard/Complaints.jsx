...
function Complaints() {
  ...
  const registerComplaint = async (e) => {
    ...
    const res = await fetch("https://hostelmanagement-ju4x.onrender.com/api/complaint/register", {
      method: "POST",
      headers: {
  ...
  const types = ["Electric", "Furniture", "Cleaning", "Plumbing", "Others"];
  ...
  useEffect(() => {
    ...
    const fetchComplaints = async () => {
      const res = await fetch("https://hostelmanagement-ju4x.onrender.com/api/complaint/student", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      <div className="flex gap-5 flex-wrap items-center justify-center">
        ...
        >
          <div>
            ...
            <select
              className="border sm:text-sm rounded-lg block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-red-500 focus:border-red-500 outline-none"
              onChange={chngType}
            >
              {types.map((type) => (
          ...
          <div>
            ...
            <button
              type="submit"
              className="w-full text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-800 text-lg rounded-lg px-5 py-2.5 mt-5 text-center"
              disabled={loading}
            >
        ...
        <div className="w-full md:w-80 max-w-md max-h-96 p-4 border rounded-lg shadow sm:p-8 bg-neutral-950 border-neutral-900 drop-shadow-xl overflow-y-auto">
          <div className="flex items-center justify-between mb-4">
            <h5 className="text-xl font-bold leading-none text-white">
              Registered Complaints Till Now
            </h5>
          </div>
