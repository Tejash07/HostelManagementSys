...
export default function RequestAcc() {
  const register = (event) => {
    ...
    fetch("https://hostelmanagement-ju4x.onrender.com/api/request/register", {
      method: "POST",
      headers: {
  ...
  const changeCM = (event) => {
    setInputCM(event.target.value);
  }
    ...
    <div className="w-full rounded-lg md:mt-0 sm:max-w-md xl:p-0 bg-gray-800 border-gray-700">
      <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
        ...
        <form className="space-y-4 md:space-y-6" onSubmit={register}>
          ...
          <button
            type="submit"
            className="w-full text-white hover:bg-red-700 focus:ring-4 focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5 text-center bg-red-600 focus:ring-red-800"
          >
            Request
          </button>
